...
async function main() {
  ...
  const wallet = { address: signer.address };
  const { AddressZero } = ethers.constants;
  const { nativeToken } = tokens;
  const weth = { address: nativeToken.address };
  const gmx = await deployContract(SYMBOL, []);
  const esGmx = await deployContract(`Es${SYMBOL}`, []);
  const bnGmx = await deployContract('MintableBaseToken', [`Bonus ${SYMBOL}`, `bn${SYMBOL}`, 0]);
  const bnAlp = { address: AddressZero };
  const alp = { address: AddressZero };

  const stakedGmxTracker = await deployContract('RewardTracker', [`Staked ${SYMBOL}`, `s${SYMBOL}`]);
  ...
  const bonusGmxTracker = await deployContract('RewardTracker', [`Staked + Bonus ${SYMBOL}`, `sb${SYMBOL}`]);
  ...
  const feeGmxTracker = await deployContract('RewardTracker', [`Staked + Bonus + Fee ${SYMBOL}`, `sbf${SYMBOL}`]);
