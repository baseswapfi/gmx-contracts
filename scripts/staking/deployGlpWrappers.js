...
const { GLP_ADDRESS, GLP_MANAGER } = require('../addresses');
...
async function main() {
  ...
  await sendTxn(
    timelock.signalSetHandler(stakedGlpTracker.address, stakedGlp.address, true),
    'timelock.signalSetHandler(stakedGlpTracker)'
  );
  await sendTxn(
    timelock.signalSetHandler(feeGlpTracker.address, stakedGlp.address, true),
    'timelock.signalSetHandler(stakedGlpTracker)'
  );
