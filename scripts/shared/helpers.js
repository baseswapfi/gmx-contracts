...
const BASE = 8453;
const BASE_GOERLI = 84532;

// const { ARBITRUM_URL, AVAX_URL, ARBITRUM_DEPLOY_KEY, AVAX_DEPLOY_KEY } = require('../../env.json');

const DEPLOY_KEY = process.env.DEV_KEY;
const providers = {
  ...
  base_goerli: new ethers.providers.JsonRpcProvider(process.env.BASE_GOERLI_RPC),
};
...
async function updateTokensPerInterval(distributor, tokensPerInterval, label) {
  ...
  if (prevTokensPerInterval.eq(0)) {
    ...
    await sendTxn(distributor.updateLastDistributionTime(), `${label}.updateLastDistributionTime`);
  }
  await sendTxn(distributor.setTokensPerInterval(tokensPerInterval), `${label}.setTokensPerInterval`);
}
