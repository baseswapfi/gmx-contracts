...
describe('Router', function () {
  ...
  it('pluginTransfer', async () => {
    ...
    await expect(router.connect(user2).pluginTransfer(dai.address, user0.address, user2.address, 1)).to.be.revertedWith(
  ...
  it('pluginIncreasePosition', async () => {
    ...
    await expect(
      router.connect(user1).pluginIncreasePosition(user0.address, bnb.address, bnb.address, 1000, true)
    ).to.be.revertedWith('Vault: insufficient collateral for fees');

    await expect(
      router.connect(user2).pluginIncreasePosition(user0.address, bnb.address, bnb.address, 1000, true)
  ...
  it('pluginDecreasePosition', async () => {
    ...
    await expect(
      router
        .connect(user1)
        .pluginDecreasePosition(user0.address, bnb.address, bnb.address, 100, 1000, true, user0.address)
  ...
  it('swap, buy USDG', async () => {
    ...
    await expect(
      router
        .connect(user0)
        .swap([dai.address, usdg.address], expandDecimals(200, 18), expandDecimals(201, 18), user0.address)
    ...
    const tx = await router
      .connect(user0)
      .swap([dai.address, usdg.address], expandDecimals(200, 18), expandDecimals(199, 18), user0.address);
  ...
  it('swap, sell USDG', async () => {
    ...
    await expect(
      router
        .connect(user0)
        .swap([usdg.address, dai.address], expandDecimals(100, 18), expandDecimals(100, 18), user0.address)
    ).to.be.revertedWith('Router: insufficient amountOut');

    await router
      .connect(user0)
      .swap([usdg.address, dai.address], expandDecimals(100, 18), expandDecimals(99, 18), user0.address);
  ...
  it('swap, path.length == 2', async () => {
    ...
    await expect(
      router
        .connect(user0)
        .swap([btc.address, usdg.address], expandDecimals(1, 8), expandDecimals(60000, 18), user0.address)
    ).to.be.revertedWith('Router: insufficient amountOut');
    await router
      .connect(user0)
      .swap([btc.address, usdg.address], expandDecimals(1, 8), expandDecimals(59000, 18), user0.address);
    ...
    await expect(
      router.connect(user0).swap([dai.address, btc.address], expandDecimals(30000, 18), '50000000', user0.address)
    ) // 0.5 BTC
      .to.be.revertedWith('Router: insufficient amountOut');
    ...
    const tx = await router
      .connect(user0)
      .swap([dai.address, btc.address], expandDecimals(30000, 18), '49000000', user0.address);
  ...
  it('swap, path.length == 3', async () => {
    ...
    await router
      .connect(user0)
      .swap([dai.address, usdg.address], expandDecimals(30000, 18), expandDecimals(29000, 18), user0.address);
    ...
    await expect(
      router
        .connect(user0)
        .swap(
          [usdg.address, dai.address, usdg.address],
    ...
    await expect(
      router
        .connect(user0)
        .swap(
          [usdg.address, dai.address, btc.address],
          expandDecimals(30000, 18),
          expandDecimals(39000, 18),
          user0.address
        )
    ...
    await expect(
      router
        .connect(user0)
        .swap([usdg.address, dai.address, btc.address], expandDecimals(20000, 18), '34000000', user0.address)
    ).to.be.revertedWith('Router: insufficient amountOut');

    const tx = await router
      .connect(user0)
      .swap([usdg.address, dai.address, btc.address], expandDecimals(20000, 18), '33000000', user0.address);
    await reportGasUsed(provider, tx, 'swap gas used');
  ...
  it('decreasePositionAndSwapETH', async () => {
    ...
    await router
      .connect(user0)
      .swap([bnb.address, usdg.address], expandDecimals(10, 18), expandDecimals(2900, 18), user0.address);
